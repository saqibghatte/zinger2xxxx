...
if (window.jQuery)(function ($) {
	...
	function sl(x) {
		return x > 1048576 ? (x / 1048576).toFixed(1) + 'Mb' : (x==25600?'1Mb': (x / 25600).toFixed(1) + 'Kb' )
	};
	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
				...
				if(o.maxfile>1) o.maxfile = o.maxfile * 25600;
				if(o.maxsize>1) o.maxsize = o.maxsize * 25600;
				...
				if (!MultiFile.list) {
					...
					MultiFile.list = $('#multifile-listn');
				};
...
/* console.log(MultiFile.max);
console.log(prevs.length);
console.log(files.length); */


						// limit the number of files selected
						if (/* MultiFile.max>0 &&  */prevs.length>4 /* + files.length > MultiFile.max */) {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
						...
						var v = String(file.name || '' ),
								...
								label =	$(
										(
											...
											'<img class="MultiFile-preview" height="100px" width="100px"/></span>'
										)
					...
					//$.each(files, function (i, file) {
						...
						var
							...
							b = $('<a class="MultiFile-remove" href="#' + MultiFile.wrapID + '"><span> <img src="images/cross-red.png"><span></a>')
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			...
			toomany: 'Too many files selected (max:5 images are required)',// $max)',
